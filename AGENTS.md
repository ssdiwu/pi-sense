...
Pi 扩展：图像仅在 active model（当前模型）不支持图片输入时，由用户指定模型描述并注入文字；本地视频 handoff 独立于 active model 的图片能力，只要启用视频并配置视频模型即处理。视频按双路径分流：普通内容理解优先走原生视频模型，涉及时间点、顺序、拖动方向等时序问题走抽帧 + 本地 ASR + 帧描述路线。
...
- 抽帧预算默认 120 帧、可通过 `/sense frames` 配置为 1–600；≤1min 0.5s/帧，>1min 在配置预算内均匀覆盖
